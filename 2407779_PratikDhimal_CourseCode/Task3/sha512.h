...
// Constants
HOST_DEVICE static const WORD k[80] = {
...
HOST_DEVICE void sha512_transform(SHA512_CTX *ctx, const BYTE data[]) {
    WORD a, b, c, d, e, f, g, h, i, j, t1, t2, m[80];

    for (i = 0, j = 0; i < 16; ++i, j += 8)
        m[i] = ((WORD)data[j] << 56) | ((WORD)data[j + 1] << 48) |
               ((WORD)data[j + 2] << 40) | ((WORD)data[j + 3] << 32) |
               ((WORD)data[j + 4] << 24) | ((WORD)data[j + 5] << 16) |
...
HOST_DEVICE void sha512_init(SHA512_CTX *ctx) {
    ctx->datalen = 0;
    ctx->bitlen = 0;
...
HOST_DEVICE void sha512_update(SHA512_CTX *ctx, const BYTE data[], size_t len) {
    size_t i;
...
HOST_DEVICE void sha512_final(SHA512_CTX *ctx, BYTE hash[]) {
    size_t i;
